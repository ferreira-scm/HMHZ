...
pdf("/SAN/Susanas_den/HMHZ/results/2020Aug/qualityProfileF1_1_2.pdf",
    height = 7, width = 7)
plotQualityProfile(fastqF[[1]])
dev.off()

pdf("/SAN/Susanas_den/HMHZ/results/2020Aug/qualityProfileR1_1_2.pdf",
    height = 7, width = 7)
plotQualityProfile(fastqR[[1]])
dev.off()

#Creation of a folder for filtrated reads
filt_path <- "/SAN/Susanas_den/HMHZ/results/2020Aug/filtered1_2"
...
if(doMultiAmp){
    ...
    filedir <- "/SAN/Susanas_den/HMHZ/results/2020Aug/stratified_files_1_2"
    ...
    saveRDS(MA, "/SAN/Susanas_den/HMHZ/results/2020Aug/MA1_2.RDS")
} else{
    MA <- readRDS("/SAN/Susanas_den/HMHZ/results/2020Aug/MA1_2.RDS")
}

trackingF=getPipelineSummary(MA)
PipSum=plotPipelineSummary(trackingF)+scale_y_log10() 
ggsave("Sequencing_summary_HMHZ_1_2.pdf", PipSum, path = "/SAN/Susanas_den/HMHZ/results/2020Aug/", height = 15, width = 15)
...
pdf("/SAN/Susanas_den/HMHZ/results/2020Aug/Estimeted_error_ratesF_1_2.pdf",
    height = 7, width = 7)
err_F
dev.off()
err_R <- plotErrors(errR, nominalQ=TRUE)
pdf("/SAN/Susanas_den/HMHZ/results/2020Aug/Estimeted_error_ratesR_1_2.pdf",
    height = 7, width = 7)
err_R
dev.off()

Heatmap <- plotAmpliconNumbers(MA)
pdf("/SAN/Susanas_den/HMHZ/results/2020Aug/heat_Sequencing_summary_HMHZ_1_2.pdf",
    height = 15, width = 15)
Heatmap
dev.off()
...
MA <- blastTaxAnnot(MA,
                    ...
                    infasta = "/SAN/Susanas_den/HMHZ/tmp/HMHZ1_2.fasta",
                    outblast = "/SAN/Susanas_den/HMHZ/results/2020Aug/blast1_2_out.fasta",
                    taxonSQL = "/SAN/db/taxonomy/update20200827/accessionTaxa.sql",
                    num_threads = 20)

saveRDS(MA, file="/SAN/Susanas_den/HMHZ/results/2020Aug/MA1_2Tax.Rds") ##Just Test run HMHZ 1

##Start from here after the taxonomic annotation
MA<- readRDS(file= "/SAN/Susanas_den/HMHZ/results/2020Aug/MA1_2Tax.Rds") ###Test run
...
saveRDS(PS, file="/SAN/Susanas_den/HMHZ/results/2020Aug/PhyloSeqCombi_HMHZ_1_2.Rds") ###Results from preliminary analysis (Sample data)
